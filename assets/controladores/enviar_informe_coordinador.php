...
$fechaInformeF = $_POST['fechaInformeF'];
$informe = $_FILES['informe'];
$fechaHoy = date('Y-m-d');
$id_archivo = 0;
$nombre_archivo = "Informe Final (coordinador)";

if($fechaInformeF !== $fechaHoy){
    echo '  
        <script>  
...
if ($informe['error'] === UPLOAD_ERR_OK) {
    ...
    $ruta_destino = $ruta_carpeta . "/Informe_final_coordinador.pdf";

    // Mover el archivo a la carpeta correspondiente
    if (move_uploaded_file($informe['tmp_name'], $ruta_destino)) {
        ...
        // Enlazar parámetros  
        $stmt->bind_param("isss", $id_carpeta, $nombre_archivo, $fechaInformeF, $ruta_destino);     
...
$tipo_notificacion = 'Informe Final Coordinador';
$mensaje = 'Subí el informe final (coordinador)';
...
$query2 = "UPDATE paso_cp SET paso = 18 WHERE id_usuario = '$codigo_a'";
...
if ($ejecutar) {  
    echo '<script>  
            alert("La ficha de evaluación fue enviado correctamente.");   
            window.location = "./../../ficha-evaluacion-alui.php?codigo=' . $codigo_a . '";
        </script>';  
} else {  
